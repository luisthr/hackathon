...
def post_gemini(json_result_properties: dict, perfil, purpose, price, bedrooms, parking_num):
...
def post_user_preferences():
    ...
    if  row_insert > 0:
        ...
        recomendetions = post_gemini(json_result_properties, user_lifetime, purpose_value, max_price, bedrooms, parking_num)
...
def get_recomendations(recomendetions, conexion_w):
    ...
    query = f"""
        SELECT property_id, score, description, sepomex_id, purpose, type_children, IF(purpose = 1, sale_price, rental_price) as price, bathrooms, bedrooms, parking_num
        FROM properties_search
